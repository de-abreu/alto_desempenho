```mermaid
flowchart TD
    A[Início] --> B(Ler dimenções x,y da imagem)
    subgraph r1 [Leitura e inicialização das variáveis]
        ...
        E --> AD(Ler endreço de imagem de destino)
        AD --> F(Ler o Seed)
        E --> G(Alocar memória para armazenar imagem)
        G --> H(**Paralelizável:** para toda linha i = 0 a f/2, alocar memória incializada a zero)
        G --> I(**Paralelizável:** para toda linha i = f/2 + 1 a x - f/2 - 1)
        G --> M(**Paralelizável:** para toda linha de i = x + f/2 a x + f, alocar memória inicilizada a zero)
    ...
    subgraph r2 [Processamento da imagem]
        ...
        Y --> Z(Compara maximos e minimos locais e atribui ao máximo e mínimo globais)
    end
